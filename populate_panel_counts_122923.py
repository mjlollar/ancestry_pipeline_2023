...
df_panel.columns = ['chr', 'pos', 'A1', 'A2', 'P1A1', 'P1A2', 'P2A1', 'P2A2']
...
df_combined.drop(df_combined.columns[[8,13,14]], axis=1, inplace=True) #drop unneeded columns(total counts, Indel and Del counts)
...
### Drop columns with no allele count and remove intermediate columns for output
index_group = df_combined[(df_combined['A1f'] == 0) & (df_combined['A2f'] == 0)].index
df_combined.drop(index_group, inplace=True)
df_combined.drop(df_combined.columns[[8,9,10,11]], axis=1, inplace=True)
